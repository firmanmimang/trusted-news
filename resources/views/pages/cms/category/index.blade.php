<x-layouts.cms.main title="Category Management">
  <x-cms.title title="Category Management" subtitle="Category on {{env('APP_NAME')}}"/>
    
  <div class="mt-8 flex justify-between items-stretch">
    <div>
      <a href="{{route('cms.category.create')}}" class="h-full inline-flex items-center text-gray-500 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-1.5 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
          <span class="sr-only">Create Category</span>
          Create Category
      </a>
    </div>
    <form action="{{route('cms.category.index')}}" method="GET" class="relative">
      @if (request('size'))
  ...
  <div class="overflow-x-auto sm:rounded-lg mt-5">
        ...
        @forelse ($categories as $category)
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
              ...
              <td class="flex justify-center items-center px-6 py-4 space-x-3">
                <a href="{{route('cms.category.edit', $category)}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                <form action="{{route('cms.category.delete', $category)}}" method="post">
                  @csrf
                  @method('DELETE')
    ...
    {{$categories->links('components.cms.table.pagination')}}
  </div>
</x-layouts.cms.main>