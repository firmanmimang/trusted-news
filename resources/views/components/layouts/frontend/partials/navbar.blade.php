<div class="text-gray-700 bg-white dark:text-gray-200 dark:bg-gray-800">
  <div x-data="{ open: false }" @click.outside="open = false" class="flex flex-col gap-2 px-4 mx-auto lg:items-center lg:justify-between lg:flex-row lg:px-5">
    <div class="flex flex-row items-center justify-between w-full py-3 lg:w-auto">
        ...
        <button x-cloak class="rounded-lg lg:hidden focus:outline-none focus:shadow-outline"
            @click="open = !open">
    ...
    <nav x-cloak :class="{ 'flex': open, 'hidden': !open }" class="flex-col flex-grow hidden lg:pb-0 lg:flex lg:justify-between lg:flex-row">
      ...
      <ul class="flex flex-col-reverse gap-5 lg:flex-row lg:items-center lg:gap-2 shrink-0">
        <li>
          <button
            x-cloak
            aria-hidden="true"
            @click="toggleDarkMode()"
            class="hidden lg:block px-4 py-2.5 mt-2 text-sm font-semibold transition duration-300 bg-transparent rounded-lg dark:bg-transparent dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
          >
        ...
        <li>
          ...
          >
            <input
              ...
              autocomplete="off"
              class="w-full px-5 pr-16 text-sm bg-white rounded-lg h-9 dark:border-0 lg:w-50 focus:outline-none focus:ring-0"
              />
            @if (request('s'))
        ...
        @auth
          <li @click.away="open = false" class="relative" x-data="{ open: false }">
            <button @click="open = !open" class="flex items-center w-full px-4 py-1 text-sm font-semibold text-left text-gray-900 transition duration-300 bg-transparent bg-gray-200 rounded-lg dark:bg-transparent dark:text-white dark:focus:text-white dark:hover:text-white dark:focus:bg-gray-600 dark:hover:bg-gray-600 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
              <span class="flex items-center gap-2">
                <img src="{{auth()->user()->imageImage}}" alt="photo profile" class="w-8 h-8 rounded-full">
                {{auth()->user()?->name}}
              </span>
              <svg fill="currentColor" viewBox="0 0 20 20" :class="{ 'rotate-180': open, 'rotate-0': !open }" class="inline w-4 h-4 mt-2 ml-1 transition-transform duration-200 transform md:mt-1">
