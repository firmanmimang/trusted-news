  ...
  <div class="flex items-center justify-between flex-grow px-4 py-4 shadow-2 md:px-6 2xl:px-11">
    ...
    <div class="flex items-center gap-3 2xsm:gap-7">
      <ul class="flex items-center gap-2 2xsm:gap-4">
        <li>
          <!-- Dark Mode Toggler -->
          <label x-cloak :class="darkMode ? 'bg-primary' : 'bg-stroke'" class="relative m-0 block h-7.5 w-14 rounded-full">
        ...
        <li class="relative" x-data="{ dropdownOpen: false, notifying: true }" @click.outside="dropdownOpen = false">
          <a class="relative flex h-8.5 w-8.5 items-center justify-center rounded-full border-[0.5px] border-stroke bg-gray hover:text-primary dark:border-strokedark dark:bg-meta-4 dark:text-white"
            ...
            <svg class="duration-300 ease-in-out fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none"
      ...
      <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
        <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
          <span class="hidden text-right lg:block">
            <span class="block text-sm font-medium text-black dark:text-white">{{auth('cms')->user()->name}}</span>
            <span class="block text-xs font-medium">
              @foreach (auth('cms')->user()->getRoleNames() as $index => $role)
                {{$role}}&nbsp;
              @endforeach
            </span>
          </span>
          ...
          <span class="w-12 h-12 overflow-hidden rounded-full">
            <img src="{{auth('cms')->user()->imageImage}}" alt="{{auth('cms')->user()->name}} profile" />
          </span>
        ...
        <div x-show="dropdownOpen"
          ...
          <ul class="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
            ...
            <li>
              <a href="{{route('cms.profile.password.edit')}}"
                class="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
                <svg class="fill-current" width="22" height="22" viewBox="0 0 22 22" fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
